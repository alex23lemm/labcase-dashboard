...
shinyServer(function(input,output){
  ...
  output$numbOfActiveProjectsCurQuart <- renderText({
    paste('Number of active projects in', quarters(dateOfExtraction), year(dateOfExtraction), ':', numbOfActiveProjectsCurQuart,sep=" ")   
  })
  ...
  output$distributionCaption <- renderText({
    paste('Departments/Countries with',input$numbOfProjects,'or more LabCase projects',sep=' ')
  })
  ...
  output$countryPlot <- renderPlot({
    ...
    g <- ggplot(country.df,aes(x=Var1,y=Freq)) + geom_bar(stat='identity',fill='#3182BD') +
      geom_text(aes(label=Freq),hjust=-0.1,size=4) +
      ylim(0, max(country.df$Freq) * 1.02) +
      xlab('Countries') + ylab('Number of Projects') + 
      theme(plot.title=element_text(size=rel(1.3)),axis.title=element_text(size=14),axis.text=element_text(size=10)) +
      coord_flip() + ggtitle('Number of LabCase projects per country')
    print(g)
  })
  ...
  output$departmentPlot <- renderPlot({
    ...
    g <- ggplot(department.df,aes(x=Var1,y=Freq)) + geom_bar(stat='identity',fill='#3182BD') +
      geom_text(aes(label=Freq),hjust=-0.1,color='black',size=4) +
      ylim(0, max(department.df$Freq) * 1.02) +
      xlab('Departments') + ylab('Number of Projects') + 
      theme(plot.title=element_text(size=rel(1.3)),axis.title=element_text(size=14),axis.text=element_text(size=10)) +
      coord_flip() + ggtitle('Number of LabCase projects per department')
    print(g)
  })
  ...
  output$projectWeekProgessPlot <- renderPlot({
    ...
    g <- ggplot(weeklyProjCreation.df,aes(x=projectsCreatedInCurrentWeek, y=Freq, group=1)) +
      geom_line(colour='#3182BD', size=1) + geom_point(size=7, shape=21, fill='white') +
      geom_text(aes(label=Freq), size=4) +
      ylim(0, max(weeklyProjCreation.df$Freq) * 1.02) +
      xlab('Day of generation') + ylab('Number of projects') + 
      theme(plot.title=element_text(size=rel(1.3)),axis.title=element_text(size=14),
  ...
  output$projectProgressPlot <- renderPlot({
    ...
    g <- ggplot(year.df, aes(x=Var1, y=Freq)) + geom_bar(width=.5, stat='identity', fill='#3182BD') +
      geom_text(aes(label=Freq), vjust=-0.3, size=4) +
      ylim(0, max(year.df$Freq) * 1.03) +
      xlab('Year of generation') + ylab('Number of projects') + 
  ...
  output$projectQuarterProgressPlot <- renderPlot({
    g <- ggplot(quarter.df,aes(x=pcy,y=Freq)) + geom_bar(width=.5, stat='identity', fill='#3182BD') +
      geom_text(aes(label=Freq),vjust=-0.3,size=4) +
      ylim(0, max(quarter.df$Freq) * 1.03) +
      xlab('Quarter of generation') + ylab('Number of projects') + 
      theme(plot.title=element_text(size=rel(1.3)),axis.title=element_text(size=14),axis.text=element_text(size=12)) +
      ggtitle(paste0(year(dateOfExtraction), ': ', 'Number of generated projects\n per quarter'))
    print(g)
  ...
  output$userSAGPlot <- renderPlot({
    ...
    # Do a reorder so that the order in the barchart is flipped
    suffix.sag.df <- transform(suffix.sag.df,suffix.sag = reorder(suffix.sag,Freq))
    
    g <- ggplot(suffix.sag.df,aes(x=suffix.sag,y=Freq)) + geom_bar(stat='identity',fill='#3182BD') +
      geom_text(aes(label=Freq),hjust=-0.2,color='black',size=4) +
      ylim(0, max(suffix.sag.df$Freq) * 1.02) +
      xlab('SAG unit') + ylab('Number of users') + 
      theme(plot.title=element_text(size=rel(1.3)),axis.title=element_text(size=14),axis.text=element_text(size=11)) +
      coord_flip() + ggtitle('Number of active SAG users per unit')
    print(g)
  ...
  output$userExternalPlot <- renderPlot({
    ...
    suffix.external.df <- transform(suffix.external.df,suffix.external = reorder(suffix.external,Freq))
    
    g <- ggplot(suffix.external.df,aes(x=suffix.external,y=Freq)) + geom_bar(stat='identity',fill='#3182BD') +
      geom_text(aes(label=Freq),hjust=-0.2,color='black',size=3) +
      xlab('Customers') + ylab('Number of users') + 
      theme(plot.title=element_text(size=rel(1.3)),axis.title=element_text(size=14),axis.text=element_text(size=11)) +
      coord_flip() + ggtitle('Customers with more than 3 active users')
    print(g)
  })
  
  output$numbOfActiveProjects <- renderText({
    paste('Number of projects active in the last 12 months:', length(activeProjects) ,sep=' ')
  })
  
  output$numbOfInactiveProjects <- renderText({
    paste('Number of projects inactive since 12 months:', dim(projects)[1] - length(activeProjects) ,sep=' ')
  })
  ...
  output$projectActivityPlot <- renderPlot({
    activeProjects.df
    g <- ggplot(activeProjects.df, aes(x=activeProjects, y=Freq)) + geom_bar(width=.5, stat='identity', fill='#3182BD') +
      geom_text(aes(label=Freq), vjust=-0.3, size=4) +
      ylim(0, max(activeProjects.df$Freq) * 1.03) +
      xlab('Quarter of last activity') + ylab('Number of projects') + 
      theme(plot.title=element_text(size=rel(1.3)), axis.title=element_text(size=14), axis.text=element_text(size=12)) +
